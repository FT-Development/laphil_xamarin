    ...
    public class MonthView : LinearLayout
    {
        ...
        public static MonthView Create(ViewGroup parent, LayoutInflater inflater, string weekdayNameFormat,
            DateTime today, ClickHandler handler, int dividerColor, int dayBackgroundResId,
                                       int dayTextColorResId, int titleTextColor, int headerTextColor,Context context)
        {
            ...
            for (int i = 0; i < 7; i++) {
                ...
                textView.SetTypeface(customfont, TypefaceStyle.Bold);
        ...
        public void Init(MonthDescriptor month, List<List<MonthCellDescriptor>> cells,Context context)
        {
            Logr.D("Initializing MonthView ({0:d}) for {1}", GetHashCode(), month);
            var stopWatch = new Stopwatch();
            stopWatch.Start();
            var boldFont = Typeface.CreateFromAsset(context.Assets, "fonts/apercu-bold-pro.otf");
            _title.SetTypeface(boldFont, TypefaceStyle.Bold);
            ...
            for (int i = 0; i < 6; i++) {
                ...
                if (i < numOfRows) {
                    ...
                    for (int c = 0; c < week.Count; c++) {
                        ...
                        cellView.SetTypeface(customfont, TypefaceStyle.Bold);
