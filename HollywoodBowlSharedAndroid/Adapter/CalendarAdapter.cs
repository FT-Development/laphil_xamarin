...
namespace HollywoodBowl.Droid
{
    class CalendarAdapter : RecyclerView.Adapter
    {
        ...
        public override void OnBindViewHolder(RecyclerView.ViewHolder holder, int position)
        {
            ...
            var date2 = localTime.ToString($"ddd, MMM dd - {TimeFormat}");
          
            viewHolder.DateTextView.Text = date2;

            if (((int)Android.OS.Build.VERSION.SdkInt) >= 24)
            {
                viewHolder.NameTextView.TextFormatted = Android.Text.Html.FromHtml(eventData.Program.Name, Android.Text.FromHtmlOptions.ModeLegacy);
            }
            else
            {
                viewHolder.NameTextView.TextFormatted = Android.Text.Html.FromHtml(eventData.Program.Name);
            }
    ...
    internal class CalendarViewHolder : RecyclerView.ViewHolder
    {
        ...
        public CalendarViewHolder(View itemView, Action<int> listener) : base(itemView)
        {
            ...
            lblSpecialRule = CalendarRow.FindViewById<TextView>(Resource.Id.lblSpecialRule);
